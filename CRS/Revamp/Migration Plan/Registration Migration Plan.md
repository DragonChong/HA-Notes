...
4. **Test Panel** — Test code input and test list display (bottom section)
...
| # | Task | Status | Notes |
|---|---|---|---|
| 0A.1 | Scaffold `lis-request-app` repository | `[x]` | CRACO + Webpack 5 + TypeScript; mirror `lab-crs-app` structure |
| 0A.2 | Configure `ModuleFederationPlugin` in `craco.config.js` | `[x]` | MF name: `RequestApp`; exposes `./Registration` |
| 0A.3 | Add `lis-request-app` to `craco.config.js` consumer list in `lis-crs-common-app` | `[x]` | `RequestApp@http://lis-chongkw-01:3012/remoteEntry.js` |
| 0A.4 | Add shared dependencies (`react`, `@cmschassis/*`, `@lis/lis-hub-lib`, MUI, Zustand, Axios) | `[x]` | Match versions in `lis-crs-common-app`; use peer deps |
| 0A.5 | Set up nginx `docker-entrypoint.sh` with `__PLACEHOLDER__` env injection | `[x]` | Dockerfile in place with `$LIS_REQUEST_SVC_URL` env injection |
| 0A.6 | Set up GitHub Actions CI/CD pipeline | `[x]` | `.github/workflows/` contains dev, release, delete workflows |
| 0A.7 | Scaffold `Registration/` folder structure under `src/features/` | `[x]` | `components/`, `hooks/`, `types/`, `api/`, `store/` sub-folders in place |
| 0A.8 | Configure Emotion scoped cache for Registration root | `[x]` | `key: "requestapp"` derived from `pluginId.toLowerCase()` in `view-handler.tsx` |

#### 0B — Integration into `lis-crs-common-app`

| # | Task | Status | Notes |
|---|---|---|---|
| 0B.1 | Register `crs-registration` view in `cms-manifest.js` | `[x]` | `id: "crs-registration"` added to `views[]` in `lis-crs-common-app/src/cms-plugin/cms-manifest.js` |
| 0B.2 | Wire `onWillDisplayView` in `plugin-manifest.module.ts` to lazy-import `./Registration` from `RequestApp` | `[x]` | `import RegistrationApp from "RequestApp/Registration"` in `lis-crs-common-app/src/modules/registration/index.tsx` |
| 0B.3 | Pass `apiContext` down to Registration component via prop or Context | `[x]` | `<RegistrationApp apiContext={cms.api as any}/>` — stored in `cms-api-provider` singleton |
...
|---|---|---|---|
| 1.1 | **HKID Input Component** — validation, format mask, lookup trigger | `[x]` | `src/components/HkidInputWrapper/` wraps `lis-hub-lib` `HkidInput` |
| 1.2 | **Encounter No. Input Component** — lookup trigger on confirm | `[x]` | `EncounterNumber` from `lis-hub-lib` used directly in `RegistrationKeys` |
| 1.3 | **Request No. Input Component** — read-only display, system-assigned | `[x]` | `RequestNumberInput` from `lis-hub-lib` used in `RegistrationKeys` |
| 1.4 | **Location Input Component** — 3-part composite (Hospital + Specialty + Ward/Sub-Spec) | `[x]` | `src/components/LocationInputWrapper/` wraps `LisLocationBox` |
| 1.5 | **Doctor Input Component** — 2-part composite (Hospital + Doctor code/name), with Create New Doctor trigger | `[x]` | `LisDoctorBox` from `lis-hub-lib` used in `RequestInfo`; Create New Doctor button wired as stub |
| 1.6 | **Test Code Input Component** — autocomplete/dropdown, multi-test panel management | `[x]` | `src/components/TestCodeInput/` component in place |
| 1.7 | **DateTime Input Component** — date+time picker with "Exact time" checkbox variant | `[/]` | CMS `DatePicker` used as placeholder in `RequestInfo`; dedicated `DateTimeInput` component not yet built |
...
|---|---|---|---|
| CRST-86 | Patient Panel | 2.2 | `[x]` |
| CRST-87 | Request Info Panel | 2.3 | `[x]` |
| CRST-88 | Test Panel | 2.4 | `[x]` |
| CRST-89 | Buttons | 2.6 | `[x]` |
...
| CRST-647 | Screen Font Size Configuration | 2.10 | `[x]` |
...
|---|---|---|---|---|
| Phase 0 — Setup | 11 | 11 | 0 | 0 |
| Phase 1 — Common Components | 7 | 6 | 1 | 0 |
| Phase 2 — Screen Layout | 11 | 7 | 1 | 3 |
...
| 2026-03-27 | Updated Phase 0, 1, 2 status based on current workspace state — Phase 0 fully complete (11/11); Phase 1 mostly complete (6/7, DateTimeInput component pending); Phase 2 partially complete (7/11, Retain panel, No. of Label, Print Tube Label, Sendout Button pending; Urgency Color in progress) |
