...
When a user enters a request number on the Wipeout Request screen that does not exist in the database, the system displays a **Request Not Found** message. The message includes the entered request number so the user can confirm which number was not found. After the user dismisses the message, the screen clears automatically and the user may try again.

> **Shared logic:** This behaviour is identical to Cancel Request. See [[Cancel Request/Workflows/Request Retrieval/Request Not Found Message|Request Not Found Message (Cancel Request)]] for the full specification.

---

## Related User Stories

- **[[CRST-986]]** - Wipeout Request - Request Not Found Message
- Full logic reference: **[[CRST-926]]** (Cancel Request - Request Not Found Message)
...
This message is shown immediately after the server returns a response confirming that no request record exists for the entered request number. It occurs as part of the request retrieval flow described in [[Retrieve Request]].
...
sequenceDiagram
    ...
    User->>Wipeout Request Screen: Enter request number and leave field
    Wipeout Request Screen->>System: Retrieve lab request data
    System-->>Wipeout Request Screen: No request record found
    Wipeout Request Screen->>User: Message 164 — "Request not found : [Request No.]"
    User->>Wipeout Request Screen: Dismiss message (OK)
    Wipeout Request Screen->>Wipeout Request Screen: Clear screen
    Wipeout Request Screen->>User: Screen returned to initial state; Request No. field cleared
```
...
|---------|------|---------|-------------|
| 164 | "Request not found : [Request No.]" | The entered request number does not exist | OK (dismiss) |
