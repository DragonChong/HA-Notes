...
**Test → Site → Spec Type → Path/Tech → Auth By → DOD → X-ray No. → Coroner → Col. Time Unknown → Gynae Clinical Data Button**

> **Note:** DOD is included in the tab sequence (the user can tab into it) but is always read-only — the field cannot be edited. It appears in the sequence between Auth By and X-ray No.
...
**Site → Spec Type → Path/Tech → Auth By → DOD → X-ray No. → Coroner → Col. Time Unknown → Gynae Clinical Data Button**
...
| DOD | **Always** (receives tab focus but is read-only — cannot be edited) |
