...
### Worksheet Code
The business code identifying a specific worksheet design, stored in the keyword's second alpha field (`key_alpha2`). This is also used to look up the sub-report name and header template from lab options.
...
This workflow is triggered automatically after the registration request has been successfully saved to the server. It is step 13 of the Registration save sequence, executed after the server confirms successful registration and after any post-registration processing has completed.
...
- For `TEST`-type keywords: at least one test profile code in the registered request matches the keyword's test profile code (`key_alpha1`).
...
sequenceDiagram
    ...
    Worksheet Determination->>Worksheet Determination: Resolve sub-report name, header template, worksheet description
...
3. For each matching keyword, the system resolves the worksheet setup:
   - **Sub-Report Name:** Retrieved from the lab option with option code `REG_WORKSHEET_SUB_REPORT_<worksheet code>` in option group `FRONT_END_REPORT`. If not found or blank, the worksheet is excluded from printing (no worksheet is generated for this code).
   - **Header Template (worksheet-specific):** Retrieved from the lab option with option code `REG_WORKSHEET_HEADER_<worksheet code>` in option group `FRONT_END_REPORT`. Used only if found and not blank.
   - **Worksheet Description:** Taken from the keyword description field (`key_desc`).
...
5. For each worksheet in the final list, the system assembles the worksheet data including patient and request information (see Data Sources section) and sends it to the backend for report generation.
...
7. If all worksheets are printed successfully, the save sequence proceeds normally to the next step.
...
#### Prerequisites
- Save button has been clicked and the registration request has been saved successfully.
- Either: no active keyword entries exist in the `REG_WS` keyword group for the current lab, OR none of the keyword entries match the registered test profiles, OR all matching keywords have no sub-report name configured.
...
1. After successful registration, the system performs the same keyword lookup and matching as Scenario 1.

2. If no worksheets qualify (no matches or no sub-report configured for any match), the system skips the printing step entirely and proceeds to the next action in the save sequence without any message to the user.
...
4. After all worksheets have been attempted (whether successfully printed or failed), the save sequence proceeds to the next step normally.
...
| Keyword Type | Condition for Inclusion | Notes |
|---|---|---|
| TEST | The registered request includes the test profile code specified in `key_alpha1` | One worksheet code per unique `key_alpha2`; duplicates skipped |
| ALL | Always included for any registration at the current lab | One worksheet code per unique `key_alpha2`; duplicates skipped |

### Header Template Resolution (Three-Tier Fallback)

| Priority | Source | Option Code | Option Group | Fallback Behaviour |
|---|---|---|---|---|
| 1 (highest) | Worksheet-specific lab option | `REG_WORKSHEET_HEADER_<worksheet code>` | `FRONT_END_REPORT` | If not found or blank, try next tier |
| 2 | Generic lab option | `REG_WORKSHEET_HEADER` | `FRONT_END_REPORT` | If not found or blank, use default |
| 3 (lowest) | System default | N/A | N/A | Use template name `RegWorksheetHeader` |

### Worksheet Data Fields on Printed Header
...
|---|---|---|---|
| 4393 | Worksheet printing failed — includes the worksheet code | Backend report generation or print service fails for a worksheet | OK (dismiss; printing continues for remaining worksheets) |
...
| Data | Source |
|---|---|
| Worksheet keyword rules | `REG_WS` keyword group — active keywords for the current lab number |
| Sub-Report Name | Lab option `REG_WORKSHEET_SUB_REPORT_<worksheet code>`, group `FRONT_END_REPORT` |
| Worksheet-specific Header Template | Lab option `REG_WORKSHEET_HEADER_<worksheet code>`, group `FRONT_END_REPORT` |
| Worksheet Description | Keyword description field |
| Global Header Template | Lab option `REG_WORKSHEET_HEADER`, group `FRONT_END_REPORT` (fallback to `RegWorksheetHeader`) |
| Sub-Report Repository | Lab option `REG_WORKSHEET_SUB_REPORT_DIR`, group `FRONT_END_REPORT` (fallback to `/appl/lis/project_files/crs_6`) |
| Request and patient data | Assembled from the registration data saved in the current request |
...
| Worksheet-specific Header Template | `REG_WORKSHEET_HEADER_<worksheet code>` | `FRONT_END_REPORT` | Overrides the header template for a specific worksheet code | This template is used for that worksheet | Falls through to generic header or default |
| Generic Header Template | `REG_WORKSHEET_HEADER` | `FRONT_END_REPORT` | Default header template used when no worksheet-specific template is configured | Used for all worksheets without a specific header option | System uses built-in default `RegWorksheetHeader` |
...
| Worksheet Keyword Rules | *(source: `REG_WS` keyword group — managed via keyword maintenance)* | — | Maps test profile codes to worksheet codes; determines which worksheets are printed | Worksheets are triggered per configured rules | No worksheets are printed |
...
3. A worksheet is only included in the print list if its sub-report name is configured in the lab option. A matching keyword without a sub-report name configuration does not produce a worksheet.
4. The header template for each worksheet follows a three-tier fallback: worksheet-specific option → generic option → built-in default `RegWorksheetHeader`.
5. Worksheets are printed silently in sequence (no print dialogue is shown to the user).
...
8. For CRS (Central Registration System) deployments, the backend resets the lab context to the specific lab number before generating the report.

---

## Related Workflows

- [[Register Request]] — Worksheet printing is triggered after the registration request has been successfully saved to the server. It is the step immediately following post-registration processing in the save sequence.
