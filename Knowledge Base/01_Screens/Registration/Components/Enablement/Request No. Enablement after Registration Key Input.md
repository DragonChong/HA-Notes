...
- [[Default Request Doctor]] — Describes the default value applied to the Req Doctor field as part of the same Ready state transition.
- [[Request No. Generation]] — When auto-generation is enabled, the Req No. field shows a placeholder mask after patient load instead of requiring manual input; the actual number is generated by the backend at save time.
