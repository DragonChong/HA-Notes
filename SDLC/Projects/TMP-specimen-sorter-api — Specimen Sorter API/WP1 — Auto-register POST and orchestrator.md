...
7. Lab not CPS or HMS → FAILURE (APS/BBS/MBS/VRS and the rest). Check with `hk.org.ha.lis.enums.Lab` from `lis-svc-lib` `lis-common` (`Lab.CPS` / `Lab.HMS`, or `Lab.fromTypeByInt` then allow only those two). Do **not** use `CommonConstants.LAB_NO_*`, data-source `hk.org.ha.lis.constants.LabType`, or CRS UDT `hk.org.ha.lis.crs.hub.model.udt.LabType`.
...
- [ ] Lab allow-list uses `hk.org.ha.lis.enums.Lab` (`CPS`, `HMS`); no `CommonConstants.LAB_NO_*`, no data-source `LabType`, no CRS UDT `LabType` on the sorter path
...
- [ ] APS / BBS / MBS (and other non CPS/HMS) → `FAILURE`, using `hk.org.ha.lis.enums.Lab` from `lis-common`
...
- **Lab check:** `hk.org.ha.lis.enums.Lab` in `lis-common` (`Lab.CPS` = 1, `Lab.HMS` = 3). `lis-crs-spec-ack-svc` already depends on `lis-common`. GNS and SOS share labNo 2 — `fromTypeByInt(2)` is not v1-allowed. Do not use `CommonConstants`, data-source `LabType`, or CRS UDT `LabType` (CHEM / HAET) for this gate.
