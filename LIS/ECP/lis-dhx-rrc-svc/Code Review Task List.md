...
| 9   | `get_latest_patient()` / `get_pmi()` / `get_patient()` | `PatientService.getLatestPatient()`       | ⚠️     | High: valid=N path throws immediately — needs fix (valid=N should route to newPatient()); Medium: updatePatientFromPMI returned null on error causing NPE on unboxing + ambiguous null on data=null in 200 response — fixed (returns API_FAIL on error, SUCCESS on 200/null-data); Low: verify getHkpmiPatientbyEnc wrapper parameter order |
