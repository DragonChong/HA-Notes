...
| 8   | `update_edi_status()`                                  | `EdiRequestService.updateEdiStatus()`     | ✅      | Low: `user_rejection_log` `AND rejected_datetime <= :createDatetime` — C passes `g_edi_rslt.create_datetime[0]` (first EDI testrslt create_datetime as upper cutoff); Java passes `System.currentTimeMillis()` (now = broader) — harmless in practice (seconds apart) — accepted; Low: `status_datetime = getdate()` (DB) vs `System.currentTimeMillis()` (app) — recurring pattern, accepted; Low: no deadlock retry loop in Java — handled at DB/framework level, accepted; `edi_request` + `edi_testrslt` UPDATE WHERE `status=98` ✅; `user_rejection_log` + `lis_rejection_log` status=99 guards ✅; `lis_rejection_log` WHERE `status<>99` ✅ |
...
| 23 | `insert_report_enquiry_cache()` | `RequestService.insertReportEnquiryCache()` | ✅ | Low: C caches `g_cfs_enable` globally (fetched once per process); Java fetches `labOption` per request — no functional difference; Low: C's "option not found" path keeps `g_cfs_enable=99` and re-fetches on next call; Java throws `RejectionLogException(11)` — stricter fail-fast is better; all 11 INSERT fields equivalent (`reqno`, `profile='MBLENQ'`, `ctr=0`, `sub_ctr=0`, `enforce_single='N'`, `cache_datetime`, `cache_file`, `pid_group`, `effective_datetime`, `expired='N'`, `last_access_datetime=null`) ✅; DELETE before skip-return and before INSERT both equivalent ✅; `option_value != 1` guard equivalent ✅ |
| 24 | `insert_tasklist()` | `RequestService.insertTasklist()` | ✅ | All 7 fields equivalent: `g_action=141` ✅; `g_sequence_no=req_labno` ✅; `g_request_time=req_registered_date` ✅; `g_request_station="RRC"` ✅; `g_request_user` (case 1,3→1050, case 7→7000) ✅; `g_reqno=req_reqno` ✅; `g_status=0` ✅; `g_message` and `g_process_time` not inserted in C (ind=-1) and not set in Java (null) ✅ |
