...
| 18 | `insert_crs_request()` | `RequestService.insertCrsRequest()` | 🔧 | **Low: age calculation uses fresh `System.currentTimeMillis()` instead of `reqRegisteredDate`** — C's `calculate_age_and_unit()` uses `req_registered_date` (the same value written to the DB) for `datediff(dd, req_dob, req_registered_date)`; Java made a second independent clock call — fixed (now passes `crsRequest.getReqRegisteredDate()` to `calculateAge()`); all field mappings verified equivalent: `req_reference=request_ward` ✅; nulls for `req_cname/req_requested_date/req_mrn/req_cdetail/req_cdetail2/req_comment` match C init defaults ✅; `req_authdoc=0`, `req_urgency=2`, `req_bill=2`, `req_diagnosis=0`, `req_retention=0` all match C ✅; `req_lab_only=1` for invalid DH requests matches C ✅; confidential PDF tag check ✅ |
