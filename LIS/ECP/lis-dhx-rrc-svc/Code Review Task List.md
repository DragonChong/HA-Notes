...
| 9   | `get_latest_patient()` / `get_pmi()` / `get_patient()` | `PatientService.getLatestPatient()`       | ⚠️     | Medium: updatePatientFromPMI null return conflates API error with logical "not found"; Low: verify getHkpmiPatientbyEnc wrapper parameter order                                                                                                                                                   |
