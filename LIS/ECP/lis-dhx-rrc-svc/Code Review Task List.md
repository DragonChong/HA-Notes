...
| 9   | `get_latest_patient()` / `get_pmi()` / `get_patient()` | `PatientService.getLatestPatient()`       | ⚠️     | High: valid=N path threw immediately (encounter null guard hit before newPatient() reached) — fixed (valid=N now routes directly to newPatient()); Medium: updatePatientFromPMI null return conflates API error with logical "not found"; Low: verify getHkpmiPatientbyEnc wrapper parameter order |
